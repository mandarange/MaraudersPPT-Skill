...
## 4.0 Phase 4 Initialization (FIRST STEP)

Before any image processing begins, initialize the output folder structure.

```bash
# Must run at Phase 4 start — before any image copy, chart render, or AI generation
...
```
{filename}_slides/
├── v{M}.{m}_{filename}.pdf           ← Written in Phase 5
├── v{M}.{m}_{filename}_appendix.pdf  ← Written in Phase 5
├── .coverage-report.json              ← Written in Phase 5
└── assets/                            ← Created at Phase 4 start
    ├── image-manifest.json            ← Created/updated throughout Phase 4
    ├── md-img-02-architecture.png     ← Priority 1: copied from source MD
    ├── chart-05-throughput.png        ← Priority 2: Playwright screenshot
    └── ai-img-07-reliability.png      ← Priority 3: LLM generated
```
...
- Extension: `.png` default, `.jpg` acceptable for photographic AI images

### Manifest Initialization

```
IF assets/image-manifest.json exists:
  -> load existing manifest (enable cache reuse)
ELSE:
  -> create new manifest with version="2.0", entries=[]
```

The manifest persists across re-runs. Cache hits are reused unless refresh is requested.
...
```json
{
  ...
  "source_md": "path/to/document.md",
  "entries": [
    {
      "slide_id": 3,
      "slide_role": "solution",
      "type": "ai-generated",
      "file_name": "ai-img-03-architecture.png",
      "file_path": "assets/ai-img-03-architecture.png",
      "visual_intent": "process-reveal",
      "prompt": "professional microservices architecture diagram with distinct service boundaries, enterprise systems context, calm confident tone, clean composition, high resolution, no text, no logos, no watermarks",
      "generator": "cursor-native",
      "width": 1920,
      "height": 1080,
      "file_size_bytes": 1243780,
      "content_hash": "sha256(normalized_prompt+generator_id+render_params)",
      "data_hash": null,
      "source_insight": "모놀리스 분리로 독립 배포 가능 - 팀 병렬화 실현",
    ...
    },
    {
      ...
      "slide_role": "proof",
      "type": "chart",
      "file_name": "chart-05-throughput.png",
      "file_path": "assets/chart-05-throughput.png",
      "visual_intent": "data-proof",
      "prompt": null,
      ...
      "content_hash": null,
      "data_hash": "sha256(normalized_chart_data+chart_type+template_version+style)",
    ...
    },
    {
      ...
      "slide_role": "hook",
      "type": "original-md",
      "file_name": "md-img-02-architecture.png",
      "file_path": "assets/md-img-02-architecture.png",
      "visual_intent": "narrative-image",
      "prompt": null,
      "generator": "source-copy",
      "width": 1200,
      "height": 800,
      "file_size_bytes": 432100,
      "content_hash": null,
      "data_hash": null,
      "source_insight": "원본 MD 이미지",
      "created_at": "ISO timestamp",
      "reusable": true
    }
  ]
...
FOR each slide requiring visual:
  ...
  lookup manifest entries by (slide_id, type)

  IF reusable=true AND hash matches AND file_path exists AND file_size_bytes > 0:
    -> CACHE HIT: reuse file_path, skip generation
  ELSE:
    -> CACHE MISS: generate/copy, update entry (overwrite on slide_id+type match)

  IF explicit refresh requested:
    -> bypass hit for selected scope
    -> regenerate and overwrite hash/timestamp/file_size_bytes
```
...
- `file_size_bytes == 0` or file not found → trigger regeneration (stale entry)
- no duplicate `(slide_id, type)` entries (overwrite on collision)
- `prompt` must be non-null and non-empty for all `type=ai-generated` entries
